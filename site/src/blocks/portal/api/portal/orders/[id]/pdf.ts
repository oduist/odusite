// GET /api/portal/orders/[id]/pdf — streams the sale order PDF from Odoo
// (auth via the Bearer token attached from the session cookie).
...
export const GET: APIRoute = async (context) => {
  ...
  if (!context.locals.user) {
    return context.redirect(`/login?next=${encodeURIComponent(`/portal/orders/${id}`)}`, 303);
  }

  try {
    const upstream = await apiFetch<Response>(context, `/my/orders/${id}/pdf`, { raw: true });
    if (!upstream.ok) {
